...
public class GraphAdjMatrix extends Graph {
    ...
    public List<Integer> getDistance2(int v) {
        ...
        int[][] distance2Matrix = squareMatrix();
        for (int column = 0; column < distance2Matrix.length; column++) {
            for (int amount = 0; amount < distance2Matrix[v][column]; amount++) {
                d2.add(column);
            }
        }
        return d2;
    ...
    private int[][] squareMatrix() {
        ...
        for (int i = 0; i < adjMatrix.length; i++) {
            for (int j = 0; j < adjMatrix.length; j++) {
                newMatrix[i][j] = multiplyMatricesCell(adjMatrix, adjMatrix, i, j);
            }
        }
        return newMatrix;
    }
